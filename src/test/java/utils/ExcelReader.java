...
public class ExcelReader {
    ...
    private static final String EXCEL_FILE_PATH = "src/test/resources/testdata/TestData.xlsx"; // Path to your Excel file
    ...
    // Load the entire sheet data into memory once
    ...
    // Utility method to safely parse integers, returning a default value if the cell is empty or invalid
    ...
    // Utility method to safely parse long, returning a default value if the cell is empty or invalid
    ...
    // Utility method to safely parse Address fields (checks for missing or empty values)
    private static Address parseAddress(Map<String, String> rowData) {
        ...
        address.setZip(parseLongSafely(rowData.get("zipCode"), 0L));
        return address;
    }
    ...
    public static User getUserByRow(int rowNumber) {
        ...
        //user.setId(parseIntSafely(rowData.get("user_id"), -1));  // -1 as default for invalid or missing ID
        ...
        user.setContact(getNewContact(rowData.get("user_contact_number")));
        
        //user.setEmail(rowData.getOrDefault("user_email_id", ""))
        //user.setContact(parseLongSafely(rowData.get("user_contact_number"), 0L)); // Default to 0 if invalid
    ...
    public static long getNewContact(String contactNumber) {
    	if (contactNumber != "")
    	{
    		String timestamp = new SimpleDateFormat("YYddHHmmss").format(new Date());	       
